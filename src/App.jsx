...
const App = () => {
  ...
  function handleRemoveFighter(remove) {
    const fighterRemove = team[remove];
    setTeam(team.filter((_, idx) => idx !== remove));
    setMoney(money + fighterRemove.price);
  }

  const totalStrength = team.reduce(
    (sum, fighter) => sum + fighter.strength, 0);

  const totalAgility = team.reduce(

  (sum, fighter) => sum + fighter.agility, 0)
